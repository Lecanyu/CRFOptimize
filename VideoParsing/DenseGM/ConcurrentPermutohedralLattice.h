/**
 * @file ConcurrentPermutohedralLattice.h
 * @brief ConcurrentPermutohedralLattice
 *
 * @author Abhijit Kundu
 */

#ifndef VIDEOPARSING_DENSEGM_CONCURRENT_PERMUTOHEDRAL_LATTICE_H_
#define VIDEOPARSING_DENSEGM_CONCURRENT_PERMUTOHEDRAL_LATTICE_H_

#include "VideoParsing/DenseGM/ConcurrentHashTable.h"
#include <Eigen/Core>

namespace vp {

template <int D>
class ConcurrentPermutohedralLattice {
  struct Neighbors {
    int n1, n2;
 ...
 public:
  ...
  ConcurrentPermutohedralLattice(const Eigen::MatrixBase<Derived>& feature, std::size_t num_of_lattice_points = 0)
    : N_(feature.cols()),
      offset_(D + 1 , N_),
      barycentric_(D + 1 , N_) {
    ...
    ConcurrentHashTable<VectorDs> hash_table(num_of_lattice_points);
    ...
    // Scale by inv Expected standard deviation of our filter (p.6 in [Adams etal 2010])
...
//      }
    ...
    // For each of d+1 axes,
	  /********************modify: j<=D -> j<D, otherwise there is segment error on n1[j] = key[j] + D ***************************/
    for (int j = 0; j < D; ++j) {
...
#pragma omp parallel for
  ...
  void compute(Eigen::MatrixBase<OutDerived> const & out, const Eigen::MatrixBase<InDerived>& in, bool reverse = false) const {
    typename InDerived::Scalar InScalar;
    ...
    if (values.size() * sizeof(InScalar)  < 160e6) {
      // Use Parallel reduction with separate copies for each thread
#pragma omp parallel
      {

        typename InDerived::PlainObject local_values(value_size, (M_ + 2));
        local_values.setZero();
...
//          local_values.col(o) += w * in.col(j);
            for (int k = 0; k < value_size; k++)
              local_values(k, o) += w * in(k, j);
          }
        }
...
#endif /* VIDEOPARSING_DENSEGM_CONCURRENT_PERMUTOHEDRAL_LATTICE_H_ */
