/**
 * @file visualizeOpticalFlow.cpp
 * @brief visualizeOpticalFlow
 *
...
int main(int argc, char **argv) {
  ...
  vp::WaitKeyNavigation navigator(0, argc - 1, true);
  do {
    const int i = navigator.value();
    cv::Mat flow = vp::readOpticalFlow(argv[i+1]);
    if (flow.empty()) {
      std::cout << "ERROR: Flow Field is empty\n";
      return EXIT_FAILURE;
    }
    cv::displayOverlay("Flow", "Flow Image #" + std::to_string(i));
    cv::imshow("Flow", vp::colorizeFlowKIITI(flow));
  } while (navigator());
